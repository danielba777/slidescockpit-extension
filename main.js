(async () => {
	...
	EXPR.vanillaVideoUrl = (haystack, options = {}) => {
		let expression = ('https?:\\/\\/(?:www\\.)?tiktok\\.com\\/@([^\\/]+)\\/(?:video|photo)\\/([0-9]+)');
	...
	itemData.extract[TTDB.MODE.FEED] = (data) => {
		...
		if (!videoData.videoApiId) {
			...
			for (const link of shareLinks) {
				...
				const matches = href ? EXPR.vanillaVideoUrl(decodeURIComponent(href)) : null;
				if (matches) {
	...
	*/
	...
	const slideshowElement = videoElement ? null : container.querySelector(slideshowSelectors.join(', '));
	const hasMediaElement = Boolean(videoElement || slideshowElement || isPhotoUrl);
	...
	*/
	const selectAllVideoItems = () => {
		...
		const selectors = DOM.multiSelector({
			...
			appSlideshowContainer: 'div[data-e2e*="browse-slide"]:not([is-downloadable])',
			...
			__nextPhoto: 'div.tt-feed div[data-e2e*="photo-card"]:not([is-downloadable])',
			__nextSlideshow: 'div.tt-feed div[data-e2e*="slide"]:not([is-downloadable])'
		});
	...
	const nextSelectors = DOM.multiSelector({
		...
		actionColumn: 'div[class*="-ActionContainer"], div[class*="-action-container"]'
	});
	...
	if (data.env === TTDB.ENV.APP) {
		...
		pipe('🔍 BROWSER mode - looking for action container with NEXT selectors');
	}
	...
	if (!linkContainer) {
		...
		pipe('✅ BROWSER mode - Action container found:', linkContainer.className);
	}
	...
	if (linkContainer) {
		...
		const button = createButton.BROWSER();
			let videoData = itemData.get(item, data) || { id: null, user: null, url: null };
		...
		if (!videoData || !videoData.id) {
			const urlMatches = EXPR.vanillaVideoUrl(window.location.href, { strict: true });
			...
			if (urlMatches) {
				...
				videoData = {
					...
					mediaType: window.location.href.includes('/photo/') ? 'slideshow' : (videoData?.mediaType || 'video')
				};
			}
		}
	...
	itemSetup.setters[TTDB.MODE.BASIC_PLAYER] = (item, data) => {
		...
		const isPhotoUrl = window.location.href.includes('/photo/');
		const slideshowElement = (videoElement && !isPhotoUrl) ? null : item.querySelector(
	...
	const input = item.querySelector('input[value*="/video/"], input[value*="/photo/"]');
	item.setAttribute('is-downloadable', 'true');
	...
	const updateItems = () => {
		...
		(allItems).forEach((item) => {
			...
			if (modeElement) {
				...
				} else if (item.querySelector('input[value*="/video/"]') || item.querySelector('input[value*="/photo/"]')) {
